...
def run_filtering(args):
    ...
    print(f"flitering raft optical flow for {scene_dir}....")
    ...
    features = [
        torch.from_numpy(
            np.load(os.path.join(scene_dir, "features", feature_name, os.path.basename(img_file) + ".npy"))
        )
        .float()
        .to(DEVICE)
    ...
    for i in range(num_imgs):
        ...
        for j in range(num_imgs):
            ...
            flow_f = np.load(os.path.join(scene_dir, "raft_exhaustive", f"{imgname_i}_{imgname_j}.npy"))
            flow_f = torch.from_numpy(flow_f).float().permute(2, 0, 1)[None].cuda()
            flow_b = np.load(os.path.join(scene_dir, "raft_exhaustive", f"{imgname_j}_{imgname_i}.npy"))
    ...
    print(f"filtering raft optical flow for {scene_dir} is done\n")
