...
class TestBalancedCrossEntropy(BaseTester):
    ...
    def test_gradcheck(self, device):
        ...
        self.gradcheck(balanced_cross_entropy_loss_with_logits, 
                       (input, target, valid), dtypes=[torch.float64, torch.float64, torch.bool])
    
    @pytest.mark.parametrize(["input", "target", "expected"], 
                             [pytest.param(
                                 torch.tensor([[[-1e+4], [1e+4]]]),
                              ...
                              pytest.param(
                                 torch.tensor([[[0.5], [0.5]]]),
                                 torch.tensor([[[0.0], [0.94]]]),
                                 logsig_m05
                              ),
                               pytest.param(
                                 torch.tensor([[[0.5], [0.5]]]),
                                 torch.tensor([[[0.10], [1.00]]]),
                                 logsig_05
                              )
                             ]
    ) 
