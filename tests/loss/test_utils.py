...
class TestMaskedMean(BaseTester):
    @pytest.mark.parametrize(
        ["mean_mode", "target_value"],
        [pytest.param("reduce_masked_mean", torch.tensor(0.75)), pytest.param("mean", torch.tensor(0.6))],
    )
    ...
    @pytest.mark.parametrize(
        ["mean_mode", "target_value"],
        [pytest.param("reduce_masked_mean", torch.tensor(0.8)), pytest.param("mean", torch.tensor(0.8))],
    )
