...
class TestEPEMetrics(BaseTester):
    @pytest.mark.parametrize(
        ["dim", "valid_dim"], [pytest.param((32, 5, 64, 64), (32, 64, 64)), pytest.param((32, 5, 64), (32, 64))]
    )
    ...
    def test_exception(self, device, dtype):
        ...
        assert (
            f"Shape of `pred` and `valid` are not paired. Expected to shape of `pred`, `valid` is (B, T, ...), (B, ...) respectivly. but Get to shape of `pred`, `valid` is {torch.Size((32, 2, 64))}, {torch.Size((32, 1))}."
            in str(errinfo)
        )
    ...
    def test_epe_all_equal_two_batch(self, device, dtype):
        pred = torch.tensor(
            [[[[0.1]], [[0.2]], [[0.3]], [[0.4]]], [[[0.1]], [[0.2]], [[0.3]], [[0.4]]]], device=device, dtype=dtype
        )
        target = torch.tensor(
            [[[[0.1]], [[0.2]], [[0.3]], [[0.4]]], [[[0.1]], [[0.2]], [[0.3]], [[0.4]]]], device=device, dtype=dtype
        )
